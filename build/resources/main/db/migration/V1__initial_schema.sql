...
-- Places table (cleaned up schema after V5 migration)
CREATE TABLE places (
    id BIGSERIAL PRIMARY KEY,
    ...
    description_source_info JSONB DEFAULT '{}',
    last_description_update TIMESTAMP WITH TIME ZONE,
    image_url TEXT,
    images TEXT[],
    gallery TEXT[],
...
CREATE INDEX idx_places_description_style ON places(description_style);
CREATE INDEX idx_places_last_description_update ON places(last_description_update);
...
-- Add vector similarity indexes (requires pgvector)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_places_keyword_vector_cosine 
    ON places USING ivfflat (keyword_vector vector_cosine_ops) 
    WITH (lists = 100);
